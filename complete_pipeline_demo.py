...
class CompletePipelineDemo:
    ...
    def test_individual_components(self):
        ...
        # Test 1: Symbolic Solver
        print("\n[1] Testing Symbolic Solver...")
        print("-" * 50)
        try:
            test_equations = [Eq(symbols('x') + symbols('y'), 50), Eq(symbols('x') - symbols('y'), 10)]
        ...
        try:
            test_equations = [Eq(symbols('x') + symbols('y'), 50), Eq(symbols('x') - symbols('y'), 10)]
            test_solution = {"x": 30, "y": 20}
            start_time = time.time()
            verification = self.verifier(test_equations, test_solution)
            ver_time = time.time() - start_time
            
            print(f"✅ Verifier: {'SUCCESS' if verification.verification['ok'] else 'FAILED'}")
            print(f"⏱ Verification Time: {ver_time:.3f} seconds")
            print(f"🔍 Verification Result: {verification.verification['ok']}")
        except Exception as e:
            print(f"❌ Verifier Error: {e}")
        
        # Test 3: LLM Reasoner
        print("\n[3] Testing LLM Reasoner...")
        print("-" * 50)
        try:
            test_query = "What is 2 + 2?"
            start_time = time.time()
            result = self.llm_reasoner(test_query, [], [])
            llm_time = time.time() - start_time
            
            print(f"✅ LLM Reasoner: {'SUCCESS' if result.success else 'PARTIAL'}")
            print(f"⏱ LLM Time: {llm_time:.3f} seconds")
            if result.llm_steps:
                print(f"🤖 Response: {result.llm_steps[:100]}...")
        except Exception as e:
